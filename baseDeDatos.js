...
//let arreglo_cookies=document.cookie.split(";") voy a reutilizar eso

/*arreglo_cookies*/

//Comento esto hasta que se agregue una verificación para que el usuario no ponga espacios al inicio de su nombre
/*
for(let i=0; i<todo_lo_guardado.length; i++);
{
  todo_lo_guardado[i]=todo_lo_guardado[i].trim();
}
console.log("Verifica Arreglo");
console.log(todo_lo_guardado);
*/

//VOYYYYYYYYYYYYYYYYYYYY a convertir moda
//moda nueva será el valor de la cookie
let arreglo_cookies=document.cookie.split(";");
let cookie_sesion_auto;
let mini_verifica;
let nombre_usuario;
let guarda_nombre_cookie_moda;
////Usuario_actual
//Recorro todos los arreglos
let moda_nueva=null;
let Usuario_actual2;
let mini_mini_verifica;
let divide_igual;
for(let hat=0; hat<arreglo_cookies.length; hat++)
{
  mini_mini_verifica=arreglo_cookies[hat].includes("ACTUAL");
  divide_igual=arreglo_cookies[hat].split("=");
  if(mini_mini_verifica)
  {
    Usuario_actual2=divide_igual[1];
    console.log("SE LOGRÓOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO");
  }
  else
  {
    console.log("QUIETOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO");
  }
}
console.log("HSDHDSFUHGDFGURUEFIDEFJHDEWHDEWUIEWUIDWEUWFIJFIFJFFEIUEJFEJEFRJIEFJIERFIJERG");
console.log(Usuario_actual2);
console.log(divide_igual);  


//si moda nuevo es undefine y la cookie de moda no tiene un número, se hace todos los artistas y albumes
for(let i=0; i<arreglo_cookies.length; i++) //Voy a guardar el valor de la moda de la cookie
{
    console.log("DEVELEOELLELELLELE");
    console.log("eRA ARRIBAAAA");
    guarda_nombre_cookie_moda=`${Usuario_actual2}_are`;
    mini_verifica=arreglo_cookies[i].includes(guarda_nombre_cookie_moda);
    //Ahora cuandoencuentre una coincidencia, la guarda en la variable
    if(mini_verifica)
    {
      moda_nueva=arreglo_cookies[i].slice(-1);             //guardo el último caracter de ese elemento, ósea la moda_nueva
    }
    else
    {
      console.log("se queda igual el usuario actual");
    }
}



//let posicion_final=todo_lo_guardado.length-1;
//console.log(posicion_final);
//id_canciones MUYYYY IMPORTANTE

let verifica_si_busco_moda=moda_nueva;
//si la moda nueva no existe, la cookie de la que proviene tampoco
if(verifica_si_busco_moda==undefined) //y por lo tanto apenas se hizo un registró o nunca se ha hecho una búsqueda
{
  console.log("Se hace todo");
}
else
{
  console.log("NO SE HACE");
  //la siguiente variable permitirá mostrar en pantalla los artistas que coincidan con el 
  //género moda (más veces puesto)
  let artista_moda=document.getElementById("contiene_artistas");   //id canciones es el arreglo
  //que guarda como elemento el genero de cada canción visitada buscada (se guarda un elemento por canción)
  //moda es el valor que más se repite en el arreglo de id_canciones

    //primero se accede al elemento específicado del arreglo genero, el cual coincide con el id 
    //proporcionado por moda
    //despues, el atributo nombre en ese elemento, me permite saber el nombre del genero 
    //que busco (el cual es el más repetido, ósea la moda)
  console.log("Antes verifica");
  console.log("Sigue prueba");
  console.log(baseDatosJSON.genero[moda_nueva].nombre)
  console.log("La cosa es: ");
  baseDatosJSON.genero[moda_nueva].nombre; //guardo el nombre del género
  //Ahora voy a buscar a TODOS LOS ARTISTAS que coinciden con ese GÉNERO de música
  let verifica_coincidencia_genero_artista;
  let link_artista_del_genero;  //el -1 es porque son 24 elementos, pero llega hasta la posición 23
  for(let i=0; i<baseDatosJSON.artistas.length-1; i++) 
  {
    
    //itero elemento por elemento buscando una coincidencia del género
    verifica_coincidencia_genero_artista=baseDatosJSON.artistas[i].genero.includes(nombre_genero_moda); 
    //lo anteror devuelve true o false
    console.log("VERIFICAREMOS QUE EXISTA MODA CON EL ARTISTA");
    if(verifica_coincidencia) //si resulta ser verdadero, guarda el artista con su foto y la muestra
    {
      //link_artista_del_genero=baseDatosJSON.artistas[i].url_img;
      //artista_moda.innerHTML+=`<img class="elemento" src="${baseDatosJSON.artistas[i].url_img}" alt="Hola Mundo">`;
      console.log("El artista que coincidió fue"+baseDatosJSON.artistas[i].nombre);
    }
    else
    {
