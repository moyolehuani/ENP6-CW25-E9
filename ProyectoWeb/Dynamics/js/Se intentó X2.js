...
//busca nombre cookie actual
let arreglo_cookie=document.cookie.split(";");
//busca nombre
let verifica_ACTUAL
let usuario_act;
let arreglo_user;
let existe_moda=null;
for(let i=0; i<arreglo_cookie.length; i++)
{
  verifica_ACTUAL=arreglo_cookie[i].includes("ACTUAL");
  if(verifica_ACTUAL)
  {
    arreglo_user=arreglo_cookie[i].split("=");
    usuario_act=arreglo_moda[1]; //segundo elemento, nombre
  }
}

console.log(usuario_act);
//obtengo cookie de moda
let verifica_moda;
let arreglo_moda;
let valor_moda=null;
for(let i=0; i<arreglo_cookie.length; i++)
{
  verifica_moda=arreglo_cookie[i].includes(`${usuario_act}_are`);
  if(verifica_moda)
  {
    arreglo_moda=arreglo_cookie[i].split("=");
    valor_moda=arreglo_moda[1]; //segundo elemento, nombre
  }
}
...
{
  let verifica_artista_generoM;
  let nombre_genero;
  let artista_contenedor=document.getElementById("contiene_artistas");
  for(let i=0; i<baseDatosJSON.artistas.length; i++) //busca artista y si encuentra coincidencia pone
  {
    nombre_genero=baseDatosJSON.genero[valor_moda].nombre;
     verifica_artista_generoM=baseDatosJSON.artista[i].genero.includes(nombre_genero);
    if(verifica_artista_generoM)
    {
      artista_contenedor.innerHTML+=`<img class="elemento" src="${baseDatosJSON.artistas[i].url_img}" alt="Hola Mundo">`;
